...
int JudgeHitMyCharNPC(NPCHAR *npc)
{
	...
	int hit_x = gMirrorMode? npc->x - 16:npc->x;

	if (gMC.y - gMC.hit.top < npc->y + npc->hit.bottom - (3 * 0x200)
		&& gMC.y + gMC.hit.bottom > npc->y - npc->hit.top + (3 * 0x200)
		&& gMC.x - gMC.hit.back < hit_x + npc->hit.back
		&& gMC.x - gMC.hit.back > hit_x)
	{
		if (gMC.xm < 0x200)
			gMC.xm += 0x200;
		hit |= 1;
	...
	if (gMC.y - gMC.hit.top < npc->y + npc->hit.bottom - (3 * 0x200)
		&& gMC.y + gMC.hit.bottom > npc->y - npc->hit.top + (3 * 0x200)
		&& gMC.x + gMC.hit.back - 0x200 > hit_x - npc->hit.back
		&& gMC.x + gMC.hit.back - 0x200 < hit_x)
	{
		if (gMC.xm > -0x200)
			gMC.xm -= 0x200;
		hit |= 4;
	...
	if (gMC.x - gMC.hit.back < hit_x + npc->hit.back - (3 * 0x200)
		&& gMC.x + gMC.hit.back > hit_x - npc->hit.back + (3 * 0x200)
...
unsigned char JudgeHitMyCharNPC3(NPCHAR *npc)
{
	...
	if (npc->direct == 0)
	{
		if (gMC.x + (2 * 0x200) > hit_x - npc->hit.front
			&& gMC.x - (2 * 0x200) < hit_x + npc->hit.back
	...
	{
		if (gMC.x + (2 * 0x200) > hit_x - npc->hit.back
			&& gMC.x - (2 * 0x200) < hit_x + npc->hit.front
			&& gMC.y + (2 * 0x200) > npc->y - npc->hit.top
			&& gMC.y - (2 * 0x200) < npc->y + npc->hit.bottom)
			return 1;
	}
...
int JudgeHitMyCharNPC4(NPCHAR *npc)
{
	...
	if (hit_x > gMC.x)
		fx1 = (float)(hit_x - gMC.x);
	else
		fx1 = (float)(gMC.x - hit_x);
	...
	if (fy1 / fx1 > fy2 / fx2)
	{
		if (gMC.x - gMC.hit.back < hit_x + npc->hit.back && gMC.x + gMC.hit.back > hit_x - npc->hit.back)
		{
