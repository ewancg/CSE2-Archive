...
void ShootBullet_Frontia1(int level)
{
	...
	if ((gbAutoFire == 2 && autofire_switch) || (gbAutoFire == 1? gKey & gKeyShot : gKeyTrg & gKeyShot))
	{
		if (!UseArmsEnergy(1))
		{
			ChangeToFirstArms();
...
void ShootBullet_Machinegun1(int level)
{
	...
	if ((gbAutoFire == 2 && autofire_switch) || (gKey & gKeyShot))
	{
		if (++gMC.rensha < 6)
			return;
...
void ShootBullet_Missile(int level, BOOL bSuper)
{
	...
	if ((gbAutoFire == 2 && autofire_switch) || (gbAutoFire? gKey & gKeyShot : gKeyTrg & gKeyShot))
	{
		if (level < 3)
		{
			if (!UseArmsEnergy(1))
...
void ShootBullet_Spur(int level)
{
	...
	else if((gbAutoFire == 2 && !autofire_switch) || !(gKey & gKeyShot))
	{
		if (spur_charge)
			bShot = TRUE;
	...
	if(gbAutoFire == 2 && !autofire_switch)
		ZeroExpMyChar();
	if (gbAutoFire != 2 && !(gKey & gKeyShot))
		ZeroExpMyChar();
...
void ShootBullet(void)
{
	...
	if(gbAutoFire == 2){
		if(gKeyTrg & gKeyShot)
			autofire_switch = !autofire_switch;
	}

	if ((gbAutoFire == 2 && autofire_switch && gArmsData[gSelectedArms].code != 13) 
		|| (gbAutoFire == 1? ((gKey & gKeyShot) && gArmsData[gSelectedArms].code != 4 && (gArmsData[gSelectedArms].code != 7 || gArmsData[gSelectedArms].level < 2) && gArmsData[gSelectedArms].code != 13): gKeyTrg & gKeyShot))
	{
		if (soft_rensha != 0)
			return;
