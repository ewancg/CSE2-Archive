...
void PutTextScript(void)
{
	...
	for (i = 0; i < 4; ++i){
		if(gColorFilters[gFilterIndex].name == "Game Boy"){
			PutBitmap4(&gTS.rcText, PixelToScreenCoord(TEXT_LEFT + text_offset), PixelToScreenCoord(gTS.offsetY + gTS.ypos_line[i] + gTS.rcText.top), &gRect_line, (SurfaceID)(SURFACE_ID_TEXT_LINE1 + i));
		}
		else{
			...
			while (!bExit)
			{
				...
				else
				{
					...
					//#endif
						...
						// Print text
						PutText2(0, -1, str, RGB(0xFF, 0xFF, 0xFE), (SurfaceID)(SURFACE_ID_TEXT_LINE1 + (gTS.line % 4)));
					#ifdef FIX_BUGS
					...
					else
					{
						...
						if (c[0] & 0x80)
						{
							...
							#else
							// Performs CP-1252 to UTF-8 conversion if necessary (This will only work for Latin-1 languages)
							if (c[0] < 0xC0)
							{
								c[1] = (char)c[0];
								c[0] = (char)0xC2;
						...
						if (c[0] == '=')
						{
							Surface2Surface(gTS.p_write * 6, 1, &rcSymbol, (SurfaceID)(SURFACE_ID_TEXT_LINE1 + (gTS.line % 4)), SURFACE_ID_TEXT_BOX);
						}
						else
						{
							PutText2(gTS.p_write * 6, -1, c, RGB(0xFF, 0xFF, 0xFE), (SurfaceID)(SURFACE_ID_TEXT_LINE1 + (gTS.line % 4)));
						}
