...
unsigned char GetAttribute(int x, int y)
{
	...
	if(gMirrorMode){
		a = *(gMap.data + x - 1 + (y * gMap.width));	// Yes, the original code really does do this instead of a regular array access
	}
...
void PutStage_Back(int fx, int fy)
{
	...
	for (j = put_y; j < put_y + num_y; ++j)
	{
		for (i = put_x; i < put_x + num_x; ++i)
		{
			...
			PutBitmap3(&grcGame, PixelToScreenCoord((i * 16) - 8) - SubpixelToScreenCoord(fx), PixelToScreenCoord((j * 16) - 8) - SubpixelToScreenCoord(fy), &rect, SURFACE_ID_LEVEL_TILESET_2);
		}
	}
}
...
void PutStage_Front(int fx, int fy)
{
	...
	for (j = put_y; j < put_y + num_y; ++j)
	{
		for (i = put_x; i < put_x + num_x; ++i)
		{
			...
			if (atrb == 0x43)
				PutBitmap3(&grcGame, gMirrorMode? (PixelToScreenCoord((i * 16) - 8) - SubpixelToScreenCoord(fx)):PixelToScreenCoord((i * 16) - 8) - SubpixelToScreenCoord(fx), PixelToScreenCoord((j * 16) - 8) - SubpixelToScreenCoord(fy), &rcSnack, SURFACE_ID_NPC_SYM);
		}
	}
}
